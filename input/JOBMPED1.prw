...
�����������������������������������������������������������������������������
/*/
static function SincFaturados(cFilMp,cCtaMp,nz)
local x
local aItens    := {} 
local cFilComp  := FWModeAccess("SF2",3)
local cFilEnt 	:= iif(cFilComp=='E',cFilMp,XFILIAL("SF2")) 

local cWherePTE := '%'
	If ExistBlock("MP_FATUFIL")
		U_ParLog1(cNomeArq, dtoc( Date() )+" "+Time()+" PTE identificado." ) 
		cWherePTE += ExecBlock("MP_FATUFIL",.F.,.F.,{aContas,cFilEnt,nz})
	EndIf				
cWherePTE += '%'

U_ParLog1(cNomeArq, " ****************************************************************************************************" )   
U_ParLog1(cNomeArq, "                                              FATURAMENTO                                            "	)	
U_ParLog1(cNomeArq, " ****************************************************************************************************" )  	
	...
	BEGINSQL alias "TMP"                          
		SELECT  MP3_IDMP,
				F2_VALBRUT,
				substring(F2_EMISSAO,1,4)+'-'+substring(F2_EMISSAO,5,2)+'-'+substring(F2_EMISSAO,7,2) as F2_EMISSAO ,
				F2_DOC,
				F2_SERIE, 
				F2_MENNOTA,
				SF2.R_E_C_N_O_ AS REC
		FROM %table:SF2% SF2,
			%table:SC5% SC5,
			%table:MP3% MP3
		...
		AND C5_FILIAL = C5_FILIAL
		AND F2_TIPO = 'N'
		AND F2_XMPTRAN <>  'I'
		AND C5_FILIAL = %EXP:cFilEnt%
		
		AND MP3.%NOTDEL%
		AND MP3_FILIAL = C5_FILIAL
		AND MP3_CTAMP = %EXP:cCtaMp%
		AND MP3_IDPROT = C5_NUM
		AND MP3_TPREG = 'PEDIDO'
	...
	while TMP->(!eof())
		aadd(aItens,{   ALLTRIM(TMP->MP3_IDMP),;
						cvaltochar(TMP->F2_VALBRUT),;
						TMP->F2_EMISSAO,;
						TMP->F2_DOC,; 
						TMP->F2_SERIE,; 
	...
	if empty(aItens)   
		...
		U_ParLog1(cNomeArq, dtoc( Date() )+" "+Time()+" Encontrado "+cvaltochar(len(aItens))+" Faturamentos ..." )    
	endif

	FOR X:=1 TO LEN(aItens)   
	
		cJson := '{'
		cJson += '  "pedido_id":'+ aItens[x,1] +','
		cJson += '  "valor_faturado":'+aItens[x,2] +','
		cJson += '  "data_faturamento": "'+aItens[x,3]+'",'
		cJson += '  "numero_nf":  "'+aItens[x,4]+'-'+aItens[x,5]+'",'
		cJson += '  "informacoes_adicionais":  "'+aItens[x,6]+'"'
		cJson += '}'
		...
		U_MP1_GRVENT(.F.,cFilMp,cCtaMp,"FATURAMENT",aItens[x,4]+aItens[x,5],cJson)
		U_ParLog1(cNomeArq, dtoc( Date() )+" "+Time()+" Faturamento "+aItens[x,4]+aItens[x,5]+ " atualizado." ) 		
